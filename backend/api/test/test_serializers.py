...
class UserSerializerTests(TestCase):
    def test_valid_user_creation(self):
        """Test user serializer creates a user with a valid password."""
        data = {
            "username": "testuser",
...
class ReceiptSerializerTests(TestCase):
    ...
    #         ]
    #     }
    #     serializer = ReceiptSerializer(data=data, context={"request": None})
    #     self.assertTrue(serializer.is_valid(), serializer.errors)
    #     receipt = serializer.save()

    #     self.assertEqual(receipt.merchant, "Test Store")
    #     self.assertEqual(receipt.total_amount, Decimal("20.50"))
    #     self.assertEqual(receipt.currency, "USD")
    #     self.assertEqual(receipt.date, "2025-02-28")
    #     self.assertEqual(receipt.payment_method, "Debit")
    #     self.assertIsNone(receipt.receipt_image_url)
    #     self.assertIsNone(receipt.group)
    #     self.assertEqual(receipt.items.count(), 2)

    def test_invalid_receipt_no_date(self):
        """Test that a receipt without a date fails validation."""
        data = {
            "user": self.user.id,
...
class FolderSerializerTests(TestCase):
    ...
    def test_create_folder(self):
        """Test creating a folder for a user."""
    ...
    def mock_request(self, user):
        """Mock request with user for testing serializers that require it."""
        class MockRequest:
...
class SubscriptionSerializerTests(TestCase):
    ...
    def test_valid_subscription(self):
        """Test subscription serializer with a user."""
        data = {
            "user": self.user.id,   
    ...
    def test_valid_subscription_with_group(self):
        """Test subscription serializer with a group."""
        data = {
            "group": self.group.id,  
            "merchant": "testmerchant",
            "total_amount": Decimal("9.99"),
